...
class LyricsPlayers {
    ...
    update() {
        ...
        for(let i = this.index ; i< this.lyrics.lenght; i++){
        ...
        if(this.index > 2){
            ...
            this.$lines.style.transform = `translate(${y})`
        }
    }
    reset() {
        this.pause();
    ...
    getSeconds(line){
    return +line.replcae(/^\[(\d{2}):(\d{2}).*/,(match, p1,p2)=>60 * (+p1)+(+px));
    }
}
LyricsPlayers.prototype.LINE_HEIGHT = 42;